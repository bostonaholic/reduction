...
- `shouldEscalate(confidence: number, graded: Graded | null): boolean` —
  ...
  ingredients (`src/core/infer.ts:403`). That dependency is stated in the
  doc comment: the grading arm does not handle rootless cards; the
  confidence arm must stay, or that case silently stops escalating.
...
1. **Root check — the single authority.** Exactly one candidate has a
   ...
   reasoning lives in exactly one place — with the guard kept, half this
   step would be unreachable and a reader could not tell which check is
   authoritative. Behaviour is unchanged: every root combination lands
   where today's code puts it. This step is also why the zero-ingredient
   vacuously-clean card (see Current state) can never win by its clean
...
5. **No-S beats any-S — presence, unconditional.** *Assumption — chosen
   ...
   `README.md:82-83`), so a sparse S-clean card sits on the sanctioned
   ladder while a broken-rich one does not. Accepted consequence, pinned in
   tests: a high-coverage card with one S finding loses to a sparse S-clean
   card. Presence, not count: ranking INVALIDs by count invents a score the
   rulebook refuses.
...
2. **Selection is the seven-step rule above.** *Assumption — chosen without
   ...
   defect: a threshold band guards one point on a continuous axis; the
...
7. **Policy lives in `src/core/escalate.ts`.** Alternatives: inline —
   ...
   inference (`src/core/grade.ts:15-18`). **Cost:** one more module, two
